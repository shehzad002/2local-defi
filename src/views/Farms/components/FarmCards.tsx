...
const FarmCards: React.FC = () => {
  ...
  const l2lIndex = farms.findIndex(
    ({ tokenSymbol }) => tokenSymbol === 'L2L',
  )

  const l2lPrice =
    l2lIndex >= 0 && stakedValue[l2lIndex]
      ? stakedValue[l2lIndex].tokenPriceInWeth
      : new BigNumber(0)

  const BLOCKS_PER_YEAR = new BigNumber(2372500)
  const L2L_PER_BLOCK = new BigNumber(60)
...
const FarmCard: React.FC<FarmCardProps> = ({ farm }) => {
  ...
  }, [l2l, lpTokenAddress, account, setHarvestable])

  const poolActive = true // startTime * 1000 - Date.now() <= 0

  return (
    <StyledCardWrapper>
      {farm.tokenSymbol === 'L2L' && <StyledCardAccent />}
      <Card>
        <CardContent>
          <StyledContent>
